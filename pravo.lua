...
local themes = import "Government Helper/imgui_themes.lua"
...
local _, id = sampGetPlayerIdByCharHandle(PLAYER_PED) -- получение айди игрока
local nickname = sampGetPlayerNickname(id) -- получение никнейма игрока
local name, surname = string.match(nickname,'(.+)_(.+)')
...
local script_url = "https://github.com/samartinell/government-helper/blob/master/pravo.luac?raw=true" -- ссылка на скрипт
...
function main()
  ...
  sampRegisterChatCommand('sex', function()sampAddChatMessage(tg..' da',main_color) end)
  ...
  while true do
    ...
    if script_crash == 'true' then print('Скрипт был принудительно завершен!') samp(crash) end
    if loading == 'true' and sampIsLocalPlayerSpawned() == true then
      ...
      sampSendChat('/mn') require('samp.events').onShowDialog = function(dialogId, style, dialogTitle, button1, button2, text)
        if dialogId == 176 then
          nowTime = text:match('Текущее время:%s+(.+)')
          roletime = 'true'
        end
        if dialogId == 63 then
        ...
        if dialogId == 27 and string.find(dialogTitle, 'Меню игрока') and loading == 'true' then
          sampSendDialogResponse(27, 1, 0, nil)
          return false
        end
        if dialogId == 0 and string.find(dialogTitle, 'Статистика игрока') and loading == 'true' then
      ...
      if org == nil then loading = 'true'
      elseif org ~= 'Правительство' and org == ".+" then
        sampAddChatMessage(tg..' Вы не являетесь сотрудником Правительства, работа скрипта невозможна.',main_color) script_crash = 'true'
      elseif ip ~= '54.37.142.74' then
        sampAddChatMessage(tg..' Скрипт предназначен только для Advance RP Blue',main_color) script_crash = 'true'
      else
        loading = 'false'
        math.randomseed(os.time())
    ...
    if getCharPlayerIsTargeting() == true and isKeyJustPressed(VK_G) then
  ...
  elseif sobes == 5 then
    ...
    elseif sobes == 10 and rank < '9' then sampAddChatMessage(tg..' Функция доступна с 9 ранга.',main_color)
      sobes = 0
    elseif sobes == 10 and rank >= '9' then
    ...
    if roletime == 'true' and mainIni.settings.rptime == true then
      sampSendChat('/me посмотрел'..feminism..' на часы марки Rollex')
      wait(300)
  ...
  elseif getCharPlayerIsTargeting(playerHandle) == true and tonumber(rank) == 6 then
    ...
    imgui.Process = imgui_help.v or interaction_window.v or imgui_st_sost.v or imgui_lawyer.v or imgui_gnews.v or imgui_licensor.v or imgui_loading.v or imgui_mainmenu.v or imgui_SB.v or imgui_leakmenu.v
...
function imgui.OnDrawFrame()
  if imgui_help.v then
    ...
    elseif menu_selected == 2 then
      ...
      imgui.SameLine()
      if imgui.Button(fa.ICON_POWER_OFF..u8'Выключить скрипт',imgui.ImVec2(191, 0)) then lua_thread.create(function() imgui_help.v = false wait(100) thisScript():exit() end) end
      imgui.EndChild()
...
if imgui_SB.v then
  ...
  if imgui_st_sost.v then
    ...
    if imgui.Button(u8'Уволить',btnsizesost) then
      if tonumber(rank) >= 8 then
        lua_thread.create(function()
          sampSetChatInputText('/uninv '..idhh..' ')
          sampSetChatInputEnabled(true)
